...
def sweep_mu_lam0_c0(argv):
    ...
    filename = Path(
        f'raw_data_'
        f'mu_min={log_mu_min}_mu_max={log_mu_max}_mu_step={log_mu_step}_'
...
def sweep_mu_a_b(argv):
    ...
    lam0_arr = np.zeros_like(log_mu_arr)
    A0_arr = np.zeros_like(log_mu_arr)
    ...
    for i, (lam_exp, A_exp) in enumerate(zip(lam_exp_arr, A_exp_arr)):
        ...
        idx_min = np.abs(lam - lam_exp).flatten().argmin()
        lam0_arr[i] = lam0.flatten()[idx_min]
        idx_min = np.abs(A - A_exp).flatten().argmin()
        A0_arr[i] = A0.flatten()[idx_min]

    mu_param = {'v_arr': mu_arr}
    lam0_param = {'v_arr': lam0_arr, 'round': 2}
    A0_param = {'v_arr': A0_arr, 'round': 2}
    ...
    grid_param = {('mu', 'lam0'): (mu_param, lam0_param, A0_param), 'a': a_param, 'b': b_param}
    ...
    filename = Path(
        ...
        f'E={np.log10(model_param.E.magnitude):.2f}_xi={np.log10(model_param.eta.magnitude/model_param.E.magnitude):.2f}_'
        f'lam={model_param.lam:.2f}_A={model_param.A:.2f}_'
        f'N={model_param.N}_dt={model_param.dt}_'
        f'T={model_param.T}.h5')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sweep',
                        choices=['mu_a_b', 'mu_lam0_c0'], help='Sweep to run')
